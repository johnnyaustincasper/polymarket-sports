                      ...
                      return (
                        <div style={{ display: 'grid', gap: 10 }}>
                          <div style={{ borderRadius: 20, padding: isMobile ? 12 : 15, background: 'linear-gradient(135deg, rgba(125,246,255,0.10), rgba(255,255,255,0.028))', border: '1px solid rgba(125,246,255,0.26)', boxShadow: activeSection ? '0 0 26px rgba(125,246,255,0.10)' : '0 0 14px rgba(125,246,255,0.04)' }}>
                            <style>{`@keyframes ufcPillHeadlight { 0% { transform: translateX(-160%) scaleX(.72); opacity: 0; } 16% { opacity: .62; } 46% { opacity: .78; } 100% { transform: translateX(235%) scaleX(.82); opacity: 0; } } @keyframes ufcPillGlow { 0%, 100% { opacity: .42; } 50% { opacity: .72; } }`}</style>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, minmax(0, 1fr))' : 'repeat(4, minmax(0, 1fr))', gap: isMobile ? 8 : 10 }}>
                              {tabs.map(tab => {
                                ...
                                return <button key={tab.key} onClick={() => setActiveKalshiSections(prev => {
                                  ...
                                  <span aria-hidden="true" style={{ position: 'absolute', inset: 3, borderRadius: 999, background: 'linear-gradient(90deg, transparent, rgba(125,246,255,0.12), rgba(255,255,255,0.11), rgba(125,246,255,0.10), transparent)', opacity: active ? 0.62 : 0.42, animation: 'ufcPillGlow 2.8s ease-in-out infinite', pointerEvents: 'none' }} />
                                  <span aria-hidden="true" style={{ position: 'absolute', top: 4, bottom: 4, left: '-28%', width: '44%', borderRadius: 999, background: 'radial-gradient(ellipse at center, rgba(255,255,255,0.62) 0%, rgba(125,246,255,0.35) 32%, rgba(125,246,255,0.10) 58%, transparent 74%)', filter: 'blur(0.4px)', mixBlendMode: 'screen', animation: `ufcPillHeadlight ${2.7 + (tabs.findIndex(t => t.key === tab.key) * 0.16)}s ease-in-out infinite`, animationDelay: `${tabs.findIndex(t => t.key === tab.key) * 0.14}s`, pointerEvents: 'none' }} />
