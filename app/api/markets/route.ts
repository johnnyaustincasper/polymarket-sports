  ...
  try {
    ...
    const games = await Promise.all(events.map(async (event: any) => {
      ...
      return {
        ...
        gameTime,
        gameDate: event.date,
        venue: (() => { const v = comp.venue; if (!v) return null; const city = v.address?.city || ''; const state = v.address?.state || ''; return { name: v.fullName || '', location: [city, state].filter(Boolean).join(', ') } })(),
