...
const C = {
  green: '#a6ff3f', red: '#ff3f5f', bg: '#030500', border: 'rgba(166,255,63,0.14)',
  borderHot: 'rgba(166,255,63,0.52)', textPrimary: '#f7fff0', textSecondary: 'rgba(219,255,191,0.58)',
}
  ...
  return <>
    ...
    {open && <div onClick={() => setOpen(false)} style={overlayStyle}>
      <aside onClick={e => e.stopPropagation()} style={panelStyle}>
        ...
        <section style={{ ...sectionStyle, padding: isMobile ? 10 : 16, borderRadius: isMobile ? 18 : 24, background: 'rgba(255,255,255,0.035)' }}><div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? 10 : 14 }}><button type="button" onClick={() => fileRef.current?.click()} style={{ width: isMobile ? 54 : 76, height: isMobile ? 54 : 76, borderRadius: isMobile ? 16 : 22, overflow: 'hidden', background: 'rgba(166,255,63,0.08)', border: `1px solid ${C.borderHot}`, color: C.green, display: 'grid', placeItems: 'center', flexShrink: 0, cursor: 'pointer', padding: 0 }}>{avatar ? <img src={avatar} alt="Profile avatar" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontWeight: 950 }}>{initials(label, profile?.email)}</span>}</button><div style={{ minWidth: 0 }}><div style={{ fontSize: isMobile ? 14 : 16, fontWeight: 950, overflow: 'hidden', textOverflow: 'ellipsis' }}>{label}</div><div style={{ color: C.textSecondary, fontSize: isMobile ? 11 : 12, marginTop: 3, overflow: 'hidden', textOverflow: 'ellipsis' }}>{profile?.email || 'Loading account…'}</div><div style={{ color: statusTone(status), fontSize: 10, fontWeight: 950, marginTop: 6, textTransform: 'uppercase' }}>{status.replace(/_/g, ' ')}</div><button type="button" onClick={() => fileRef.current?.click()} disabled={saving} style={{ marginTop: 8, borderRadius: 999, border: `1px solid ${C.border}`, background: 'rgba(166,255,63,0.07)', color: C.green, padding: '7px 10px', fontSize: 10, fontWeight: 950, letterSpacing: '0.10em', textTransform: 'uppercase', cursor: saving ? 'wait' : 'pointer' }}>{saving ? 'Uploading…' : 'Upload photo'}</button><input ref={fileRef} type="file" accept="image/*" onChange={e => uploadProfilePicture(e.target.files?.[0])} style={{ display: 'none' }} /></div></div></section>
        <section style={{ display: 'grid', gap: isMobile ? 7 : 10, marginBottom: isMobile ? 9 : 14 }}>{[
          ...
          <button onClick={saveProfile} disabled={saving} style={{ borderRadius: isMobile ? 13 : 16, border: `1px solid ${C.borderHot}`, background: 'linear-gradient(135deg, rgba(166,255,63,0.95), rgba(197,255,93,0.78))', color: C.bg, padding: isMobile ? '11px' : '14px', fontSize: 12, fontWeight: 950, letterSpacing: '0.12em', textTransform: 'uppercase', cursor: saving ? 'wait' : 'pointer' }}>{saving ? 'Saving…' : 'Save Profile'}</button></section>
        <section style={sectionStyle}><div style={{ color: C.textSecondary, fontSize: 10, fontWeight: 950, letterSpacing: '0.16em', textTransform: 'uppercase' }}>Subscription</div><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginTop: isMobile ? 7 : 10 }}><div><div style={{ color: statusTone(status), fontSize: isMobile ? 15 : 18, fontWeight: 950, textTransform: 'capitalize' }}>{status.replace(/_/g, ' ')}</div><div style={{ color: C.textSecondary, fontSize: isMobile ? 11 : 12, marginTop: 3 }}>{profile?.guest ? 'Temporary full-access testing pass.' : profile?.hasStripeCustomer ? 'Stripe billing connected.' : 'No Stripe customer linked yet.'}</div></div><span style={{ width: 10, height: 10, borderRadius: '50%', background: statusTone(status), boxShadow: `0 0 16px ${statusTone(status)}` }} /></div><button onClick={manageBilling} disabled={billingLoading || profile?.guest} style={{ width: '100%', marginTop: isMobile ? 9 : 14, borderRadius: isMobile ? 13 : 16, border: `1px solid ${C.border}`, background: profile?.guest ? 'rgba(255,255,255,0.035)' : 'rgba(166,255,63,0.08)', color: profile?.guest ? C.textSecondary : C.green, padding: isMobile ? '11px' : '13px', fontSize: 12, fontWeight: 950, letterSpacing: '0.12em', textTransform: 'uppercase', cursor: billingLoading ? 'wait' : profile?.guest ? 'not-allowed' : 'pointer' }}>{billingLoading ? 'Opening…' : profile?.guest ? 'Billing unavailable for guests' : 'Manage Billing'}</button></section>
