...
function buildMlbMatchupRating(input: {
  ...
  if (kind === 'strikeouts') {
    ...
    return {
      ...
      rows: [
        ...
        `Matchup gap: ${strikeouts - lineupContactRisk >= 10 ? '+' : ''}${strikeouts - lineupContactRisk}; this only gets strong if opponent strikeouts, pitch count, and umpire zone agree.`,
  ...
  return {
    ...
    opponentLabel: pitcherVulnerability != null ? 'Pitcher weakness' : 'Pitcher setup',
    ...
    rows: [
      ...
      `Best prop fit: ${fits[0][0]} (${fits[0][1]}/100); homers only deserve attention when the power setup is better than the normal contact setup.`,
...
function buildMlbMisreadSignal(metric: string, rating?: MlbMatchupRating): MlbMisreadSignal | undefined {
  ...
  if (kind === 'strikeouts') {
    return {
      ...
      matchupGap: gap,
      summary: `${rating.ratingTitle}: ${rating.playerRating} vs ${rating.opponentRating} opponent contact`,
      reason: `This stands out because the pitcher profile is stronger than this opponent's contact profile${rating.opponentProof?.[0] ? ` — ${rating.opponentProof[0]}` : ''}. It still needs pitch count and umpire support.`,
  ...
  if (kind === 'power') {
    return {
      ...
      opponentRating: rating.opponentRating,
      matchupGap: gap,
      summary: `${rating.ratingTitle}: ${rating.playerRating} vs ${rating.opponentRating} pitcher weakness`,
      reason: `This stands out because the power path fits the pitcher/park setup${rating.opponentProof?.[0] ? ` — ${rating.opponentProof[0]}` : ''}; homers only upgrade when the power case is stronger than the normal contact case.`,
      ratingTitle: rating.ratingTitle,
      bestFit: rating.bestFit,
      subRatings: rating.subRatings,
      opponentProof: rating.opponentProof,
  ...
  if (kind === 'offense') {
    return {
      ...
      label: 'Run-path matchup flag',
      severity,
      playerRating: rating.playerRating,
      opponentRating: rating.opponentRating,
      matchupGap: gap,
      summary: `${rating.ratingTitle}: ${rating.playerRating} vs ${rating.opponentRating} pitcher weakness`,
      reason: `This stands out because the lineup role and opposing starter/park setup look better than the surface read${rating.opponentProof?.[0] ? ` — ${rating.opponentProof[0]}` : ''}.`,
      ratingTitle: rating.ratingTitle,
      bestFit: rating.bestFit,
      subRatings: rating.subRatings,
      opponentProof: rating.opponentProof,
  ...
  return {
    kind: 'hitter_contact',
    label: 'Contact matchup flag',
    severity,
    playerRating: rating.playerRating,
    opponentRating: rating.opponentRating,
    matchupGap: gap,
    summary: `${rating.ratingTitle}: ${rating.playerRating} vs ${rating.opponentRating} pitcher weakness`,
    reason: `This stands out because the hitter profile and opposing starter weakness both point the same way${rating.opponentProof?.[0] ? ` — ${rating.opponentProof[0]}` : ''}.`,
    ratingTitle: rating.ratingTitle,
    bestFit: rating.bestFit,
    subRatings: rating.subRatings,
    opponentProof: rating.opponentProof,
...
function buildMlbConviction(input: {
  ...
  if (kind === 'strikeouts') {
    return {
      verdict,
      read: `${input.player} is a pitcher strikeout read, not just a box-score play: ${fmt(input.lastGame.value)} Ks last start, ${fmtAvg(input.last5Avg)} over the last ${input.games5}, with a ${recentBand} band.`,
      whyLive: [
        ...
        `The path is pitch count plus swing-and-miss: he needs enough innings for the slider/secondary stuff to pile up Ks.`,
        `Umpire zone and opponent contact matter more here than a generic recent trend.`,
      ],
      ...
      numberDiscipline: input.line == null ? 'Needs a clear strikeout number before it belongs on the board.' : `${currentProp} is the clean line; one K higher is picky, two higher needs a perfect matchup.`,
      matchupRating,
      misreadSignal,
      opponentProof,
    }
  ...
  if (kind === 'power') {
    return {
      ...
      numberDiscipline: 'Home run looks are naturally thin; do not turn good hitter form into a homer chase unless the matchup supports it.',
      matchupRating,
      misreadSignal,
      opponentProof,
    }
  ...
  if (kind === 'offense') {
    return {
      ...
      numberDiscipline: `${currentProp} only makes sense when the lineup card confirms the role; do not chase a bigger number without a strong team run setup.`,
      matchupRating,
      misreadSignal,
      opponentProof,
    }
  ...
  return {
    ...
    numberDiscipline: `${currentProp} is the clean line; if the app asks for a bigger night, require a confirmed lineup and better matchup support.`,
    matchupRating,
    misreadSignal,
      opponentProof,
  }
