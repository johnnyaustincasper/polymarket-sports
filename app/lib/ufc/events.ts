...
export async function fetchPolyUFCEvents(): Promise<any[]> {
...
export function safeArray<T = any>(raw: unknown): T[] {
...
export function getPolyOddsForFight(fighterA: string, fighterB: string, polyEvents: any[]): UFCPolyOdds {
...
export function parseFighter(competitor: any, rankingsMap: Record<string, number>): UFCFighter {
...
export function parseEventFromScoreboard(ev: any, rankingsMap: Record<string, number>, polyEvents: any[] = []): UFCEvent {
  const id = String(ev.id || ev.uid || '')
  const name = ev.name || ev.shortName || 'UFC Event'
  const dateRaw = ev.date || ev.competitions?.[0]?.date || ''
  const dateTime = dateRaw ? new Date(dateRaw).getTime() : NaN
  const date = Number.isFinite(dateTime) ? new Date(dateTime).toISOString() : ''
  ...
  for (let i = 0; i < competitions.length; i++) {
    ...
    if (oddsArr.length > 0) {
      ...
      const homeMoneyLine = rawHome !== null && rawHome !== undefined && Number.isFinite(Number(rawHome)) ? Number(rawHome) : null
      const awayMoneyLine = rawAway !== null && rawAway !== undefined && Number.isFinite(Number(rawAway)) ? Number(rawAway) : null
      const fighterAHomeAway = String(competitors[0]?.homeAway || '').toLowerCase()
      const fighterBHomeAway = String(competitors[1]?.homeAway || '').toLowerCase()
      if (fighterAHomeAway === 'home') moneyLineA = homeMoneyLine
      else if (fighterAHomeAway === 'away') moneyLineA = awayMoneyLine
      else moneyLineA = homeMoneyLine
      if (fighterBHomeAway === 'home') moneyLineB = homeMoneyLine
      else if (fighterBHomeAway === 'away') moneyLineB = awayMoneyLine
      else moneyLineB = awayMoneyLine
    }
...
export async function fetchUFCEvents(): Promise<UFCEvent[]> {
